...
export default function RevokeEntryForm({
  loading,
  onSubmit,
  onClose,
}: RevokeEntryFormProps) {
  const { 0: formData, 1: setFormData } = useState<RevokeBlacklistEntryData>({
    revocationReason: "",
  });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg max-w-md w-full mx-4">
        <h3 className="text-xl font-semibold mb-4">
          Отменить запись в черном списке
        </h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setFormData({ ...formData, revocationReason: e.target.value })
              }
