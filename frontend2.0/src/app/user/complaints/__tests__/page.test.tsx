...
import UserComplaintsPage from '../page';
...
jest.mock('@/components/Complaints/ComplaintsManagement', () => {
  return function MockedComplaintsManagement({ userRole }: { userRole: UserRole }) {
    return (
      <div data-testid="complaints-management" data-user-role={userRole}>
        Mocked Complaints Management Component
      </div>
    );
  };
});
...
describe('UserComplaintsPage', () => {
  ...
  describe('Page Rendering', () => {
    test('renders page with proper layout for authorized visitor', async () => {
      ...
      await waitFor(() => {
        const container = screen.getByTestId('complaints-management').closest('.min-h-screen');
        expect(container).toHaveClass('min-h-screen', 'bg-background');
        
        const innerContainer = screen.getByTestId('complaints-management').closest('.container');
        expect(innerContainer).toHaveClass('container', 'mx-auto', 'px-4', 'py-8');
      });
    });

    test('passes correct user role to ComplaintsManagement component', async () => {
      mockUseAuthStore.mockReturnValue({
        user: {
          id: 'visitor-123',
          email: 'visitor@example.com',
      ...
      await waitFor(() => {
        const complaintsComponent = screen.getByTestId('complaints-management');
        expect(complaintsComponent).toHaveAttribute('data-user-role', UserRole.VISITOR);
      });
    });

    test('renders ComplaintsManagement component', async () => {
      mockUseAuthStore.mockReturnValue({
        user: {
          id: 'visitor-123',
          email: 'visitor@example.com',
      ...
      await waitFor(() => {
        expect(screen.getByText('Mocked Complaints Management Component')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Security and Access Control', () => {
    test('returns null for unauthorized users after loading', async () => {
      ...
      const { container } = render(<UserComplaintsPage />);
  ...
  describe('Component Lifecycle', () => {
    test('handles auth state changes correctly', async () => {
      ...
      const { rerender } = render(<UserComplaintsPage />);
  ...
  describe('Accessibility', () => {
    test('has proper semantic structure', async () => {
      ...
      await waitFor(() => {
        const mainContainer = screen.getByTestId('complaints-management').closest('.min-h-screen');
        expect(mainContainer).toBeInTheDocument();
        
        const contentContainer = screen.getByTestId('complaints-management').closest('.container');
  ...
  describe('Page Consistency', () => {
    test('maintains consistent styling with other user pages', async () => {
      ...
      await waitFor(() => {
        ...
        const pageContainer = screen.getByTestId('complaints-management').closest('.min-h-screen');
        expect(pageContainer).toHaveClass('min-h-screen', 'bg-background');
        
        const contentWrapper = screen.getByTestId('complaints-management').closest('.container');
